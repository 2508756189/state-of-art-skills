...
Synced from `supermemoryai/supermemory` at commit `d7050ed332c7894599551ef3382a05f2dc117e5e`, path `skills/supermemory`, with a local safety overlay for this Skill Market. Upstream `README.md` is intentionally omitted because it duplicates Claude/API installation prose rather than Codex runtime instructions.
...
Treat Supermemory APIs and SDKs as optional unless the user explicitly asks to build against the live service and a safe credential path is already configured.

Never print, persist, or commit API keys, tokens, cookies, private account entries, one-time auth codes, full customer exports, or live credentials from config files. Store only stable, reusable facts such as repo locations, verified commands, service topology without secrets, decisions and rationale, recurring error signatures, and safe schema or report-shape facts.

When using memory-derived facts, distinguish them from currently verified facts. Re-verify drift-prone details before acting on production systems, credentials, schedules, model/provider availability, pricing, or live service state.
